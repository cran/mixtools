    ...
    epsilon = 1e-08, verb = FALSE) 
{
    ...
    sing <- sum(lambda<1e-08)*(k>1)
            if(diff<0 || sing>0 || abs(newll)==Inf || is.nan(newll) || sum(postz)!= n){
...
rownames(theta) <- c(paste("theta", ".", 1:p, sep = ""))
colnames(theta) <- c(paste("comp", ".", 1:k, sep = ""))
colnames(postz) <- c(paste("comp", ".", 1:k, sep = ""))
    cat("number of iterations=", iter, "\n")
    list(y=y, lambda = lambda, 
        theta = theta, loglik = sum(log(dens)), posterior = postz, all.loglik=ll, ft="multmixEM")
}
