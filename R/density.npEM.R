...
density.npEM <- function(x, u=NULL, component=1, block=1, scale=FALSE, ...) {
  ...
  if (!is.null(x$symmetric) && x$symmetric) {
    # location mixture; must recenter data once for each component's
    # value of mu.  Also recenter u for only the selected component.
    n <- length(xx) # assuming r=1
    d <- wkde(x = rep(xx, m) - rep(x$muhat, each=n), 
              u = u - x$muhat[component],
              w = as.vector(x$post), bw=bw, sym=TRUE)
  } else {
    n <- NROW(x$data)
    wts <- rep(x$post[,component], sum(coords))
    d <- wkde(xx, u=u, w=wts, bw=bw)
  }
  if (scale) d <- d*x$lambdahat[component]
