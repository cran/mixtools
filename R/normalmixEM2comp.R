# fast function for two-component normal mixture with equal variances.

normalmixEM2comp <- function(x, lambda, mu, sigsqrd, eps=1e-8, maxit=1000) {
  ...
  out <- list(x=x, lambda = c(lambda,1-lambda), mu = c(mu1, mu2), 
       ...
       restarts=0, ft="normalmixEM2comp")
  class(out) <- "mixEM"
  out
}
