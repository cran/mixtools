...
\Address{
Didier Chauveau\\
Laboratoire MAPMO - UMR 6628 - F\'ed\'eration Denis Poisson\\
...
of a (multivariate) nonparametric situation is $\CF=\{\phi(\vec x) =
...
be used to find a local maximum of the likelihood surface.  The E-step
...
\code{kernl.g = 3}. The bandwidth we selected was $h=25$, which we
specify with \code{kernl.h = 25}.

For this implementation of the IGLE algorithm, we set the parameter
estimates obtained from the mixture of regressions EM algorithm as
starting values for $\hat{\vec{\beta}}_{1}$, $\hat{\vec{\beta}}_{2}$,
$\hat{\sigma}_{1}$, and $\hat{\sigma}_{2}$, and set the starting values
for $\lambda(x_{i})$ to be 0.5 for all $x_{i}$.
<<CO2igle>>=
CO2igle <- regmixEM.loc(CO2, GNP, beta = CO2reg$beta, sigma = CO2reg$sigma,
                        lambda = matrix(.5, 28, 2), kern.l = "Beta",
                        kernl.h = 25, kernl.g = 3)
@
