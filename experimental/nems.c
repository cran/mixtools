...
void nems_Mstep_v1(
  ...
  int n=*nn, m=*mm, r=*rr, ngrid=*nngrid, i, j, k, a;
  double sum, xik, *fjk, two_h_squared =2*(*hh)*(*hh);
  double denom, pij, gridpt;
  double normconst = 0.39894228040143267794/ *hh; /* 0.3989...=1/(sqrt(2*pi)) */

  for (j=0; j<m; j++) {   /* for each component */
    for (k=0; k<r; k++) { /* for each coordinate */
      fjk = new_f + ngrid*(j + m*k); /* fjk is now an array of size ngrid */
      for (a=0; a<ngrid; a++) {
        ...
        for (i=0; i<n; i++) {
          ...
          /* Numerator: Take p_{ij} times the normal density at (x_{ik}-gridpt) */
          denom += pij;
          /* Denominator:  Add all the p_{ij}.  This is the same as n*lambda[j],
             but we do not have access to the lambda vector in this function. */
        }
        fjk[a] = normconst * sum / denom ; 
        /* Note that each fjk "density" should now be correctly normalized;
           normconst is the normalizing constant for a normal kernel function */           
      }
    }
  }
}      
  ...
  for (i=0; i<n; i++) {
    ...
    for (j=0; j<m; j++) {
      ...
      for (k=0; k<r; k++) {
        ...
        conv = 0.0;
        for (a = 0; a<ngrid; a++) {
          conv += exp(-(xik - grid[a])*(xik-grid[a])/two_h_squared) * log(fjk[a]);
        }
...
/* ************************************************************
   version 2: checking underflows in numerical integration 
   called by nems.v2() R function until final version 
   only nems_Estep_v2 has been changed from first version */
...
void nems_Estep_v2(
  ...
  double Delta = (grid[2]-grid[1]) / *hh / sqrt(2*3.14159265358979);
  double t1, t2;
  double epsi=1e-323;	/* smallest number machine-dependent? */
  ...
  for (i=0; i<n; i++) {
    ...
    for (j=0; j<m; j++) {
      ...
      for (k=0; k<r; k++) {
		...
		if (fjk[a] > epsi) { /* no underflow pb */
			conv += t1 * log(fjk[a]);
		}
		else if (t1 < epsi2) { /* assume kernel cancels log(0) part */
			*nb_udfl +=1;
		}
		else *nb_nan +=1; /* kernel NOT small enough ? */
       }
        conv *= Delta; /* Delta is the normalizing constant */
        conv = exp(conv); /* conv now = Nf_jk(xik) */
        post[i + n*j] *= conv; /* numerator = lambda_j*prod_k Nf_jk(xik) */
      }
      sum += post[i + n*j];
    }
    *loglik += log(sum);
...
/* **********************************************************
   VERSION 3 allowing blocks of conditionaly iid coordinates: 
	 ...
	 if (blockid[k] == (ell + 1)) {  /* coordinate k is in block ell */
         ...
         for (i=0; i<n; i++) {
	      ...
	      times the normal density at (x_{ik}-gridpt) */
           /* denom += pij; */
           /* obsolete: computed from n*lambda[j]*BlS[ell] 
